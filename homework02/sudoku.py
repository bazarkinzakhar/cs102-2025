...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[T]]:
    ...
    [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    """
    result = [[values[i] for i in range(j, j + n)] for j in range(0, len(values), n)]
    return result
...
def get_row(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    """
    x, y = pos
    row = [grid[x][i] for i in range(len(grid))]
    return row
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    used_digits = set()
    used_digits.update(get_row(grid, pos))
    used_digits.update(get_col(grid, pos))
    used_digits.update(get_block(grid, pos))
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    new_grid = [["."] * 9 for _ in range(9)]
    for i, j in positions[:N]:
